<main>
    ...
    <form class="form form--add-lot container <?=$form_invalid?>" action="add.php" method="post" enctype="multipart/form-data"> <!-- form--invalid -->
      ...
      <div class="form__container-two">
        <div class="form__item <?=$field_invalid['lot-name']?>"> <!-- form__item--invalid -->
          <label for="lot-name">Наименование<?=$errors['lot-name']?></label>
        ...
        <div class="form__item <?=$field_invalid['category']?>">
          <label for="category">Категория<?=$errors['category']?></label>
      ...
      <div class="form__item form__item--wide <?=$field_invalid['description']?>">
        <label for="message">Описание<?=$errors['description']?></label>
      ...
      <div class="form__container-three">
        <div class="form__item form__item--small <?=$field_invalid['lot-rate']?>">
          <label for="lot-rate">Начальная цена<?=$errors['lot-rate']?></label>
          <input id="lot-rate" type="number" name="lot-rate" placeholder="0" value="<?=$filled_field_array['lot-rate']?>" required>
          <span class="form__error">Введите начальную цену</span>
        </div>
        <div class="form__item form__item--small <?=$field_invalid['lot-step']?>">
          <label for="lot-step">Шаг ставки<?=$errors['lot-step']?></label>
          <input id="lot-step" type="number" name="lot-step" placeholder="0" value="<?=$filled_field_array['lot-step']?>" required>
          <span class="form__error">Введите шаг ставки</span>
        </div>
        <div class="form__item <?=$field_invalid['lot-date']?>">
          <label for="lot-date">Дата окончания торгов<?=$errors['lot-date']?></label>
