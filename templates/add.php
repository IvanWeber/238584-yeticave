<form class="form form--add-lot container <?php if ($form_invalid) {
    ...
    <div class="form__container-two">
        <div class="form__item <?php if (!empty($field_invalid['lot-name'])) {
            print('form__item--invalid');
        } ?>"> <!-- form__item--invalid -->
            <label for="lot-name">Наименование</label>
            <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота"
                   value="<?= $filled_field_array['lot-name'] ?>" required>
            <span class="form__error">Введите наименование лота</span>
        </div>
        <div class="form__item <?php if (!empty($field_invalid['category'])) {
            print('form__item--invalid');
        } ?>">
    ...
    <div class="form__item form__item--wide <?php if (!empty($field_invalid['description'])) {
        print('form__item--invalid');
    } ?>">
        <label for="message">Описание</label>
        <textarea id="message" name="description" placeholder="Напишите описание лота"
                  required><?= $filled_field_array['description'] ?></textarea>
    ...
    <div class="form__item form__item--file"> <!-- form__item--uploaded -->
        ...
        <div class="form__input-file">
            ...
            <label for="photo2">
                <?php if ($image_invalid): ?>
                    <span style="color: red; font-size: 12px;">Загрузите изображение лота</span>
                <?php else: ?>
                    <span>+ Добавить</span>
                <?php endif; ?>
            </label>
    ...
    <div class="form__container-three">
        <div class="form__item form__item--small <?php if (!empty($field_invalid['lot-rate'])) {
            print('form__item--invalid');
        } ?>">
            <label for="lot-rate">Начальная цена</label>
            <input id="lot-rate" type="number" name="lot-rate" placeholder="0"
                   value="<?= $filled_field_array['lot-rate'] ?>" required>
            <span class="form__error">Введите начальную цену</span>
        </div>
        <div class="form__item form__item--small <?php if (!empty($field_invalid['lot-step'])) {
            print('form__item--invalid');
        } ?>">
        ...
        <div class="form__item <?php if ($field_invalid['lot-date']) {
            ...
            <input class="form__input-date" id="lot-date" type="date" name="lot-date"
                   value="<?= $filled_field_array['lot-date'] ?>" required>
